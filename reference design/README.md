...
**Contents:** `README.md` (this file, everything in one place), `tokens.json`, `bundle.css`, `library.md` (the `c_*` controls), `components/<Name>/` (README.md and a self-contained preview.html per component) `screenshots/<Name>.png`, `index.html` (open it to browse every component), `icons/` (one SVG per icon), `icons.md` and `_icons.html` (the icon sheet).
...
Design on a 640 x 360 (16:9) reference and lay it out so it also fits wider screens: the height is fixed at 360 units and the width is whatever the device has (640 at 16:9, about 800 at 20:9). The top bar, tool rail and side panel keep their sizes; **the arena or canvas takes all the width that is left**, so extra width becomes play area, never wider panels or bigger controls. **UI size** (Settings) multiplies the complete token set, including the `touch` token, so the screen holds fewer units: 200% leaves 320 x 180 on a 16:9 device. Nothing has a separate hard-coded minimum outside that scale; when the arena would get narrower than 200 units the side panel collapses to its 28px tab (see Build). Inset for the display cutout and gesture bar. At 100%, every top bar and touch target is 48px with the same shape: Back, title, spacer, at most two icons and an overflow. Below it an arena or canvas on the left and one fixed side panel (168 to 176px) on the right; Build and BuildLocked have a 56px tool rail on the left. Keep 8px (`space-2`) from screen edges.
...
Interactive control glow has one shared definition (`--glow`; `UiGlow` in Godot). Buttons, selected controls and slider thumbs use that same colour, opacity and extent rather than defining local shadows.
...
This touch-first prototype does not define a separate focus-ring state. Selected controls use their documented `on` or selected treatment; editing text fields use their editing state. Nothing depends on hover.

**For Godot.** Each control here is one scene, built once and reused: `Button` (one scene: kind default, primary, danger or flat; layout row, icon or stack; state on or off; StyleBoxFlat `raised`, with the primary and on looks as variants, and `off` as a dashed border at 50% alpha), `Slider` (a custom `Control`, configured by properties: `thumbs` as 0.0 to 1.0 (one, or two for a range, the same scene with a second thumb), `steps` as an Array of at least two labels laid out evenly, `marker` as a position 0.0 to 1.0 and a name, `enabled`; the marker name is a Label in the label row, centred over the marker and clamped between the label and the value by their measured widths; the minus and plus buttons of a stepped slider are an HBoxContainer around it), `Toggle` and `Checkbox` (CheckButton, CheckBox), `Segmented` (a row of toggle Buttons in a ButtonGroup), `Picker` (a Button that opens a PopupMenu), `Menu` (PopupPanel), `Chip` (PanelContainer with a Label), `Dialog` (a modal PanelContainer). Components currently consume the shared `UiTokens` adapter. The native Godot `Theme` resource and automatic theme propagation are the target tracked by #236: `space-1` to `space-5` become `separation` and `margin` constants, `control-xs` to `touch` become minimum-size constants, and radii, colours and typography become theme values. Containers own layout; no screen positions a control by hand except over the arena canvas.

**One implementation each.** Every entry above is one component in the kit and every screen calls it: `c_textfield`, `c_toggle` (with a `dense` variant that is a `control-sm` row with a smaller track, for settings panels), `c_seg`, `c_slider(label, value, thumbs, steps, marker, enabled)` (`c_range` is the same slider with two thumbs; `stepped_slider` is only the layout with minus and plus buttons), `c_pick`, `c_value`, `c_readonly`, `c_meter`, `c_power`, `c_row`, `c_tabs`, `c_panel_head`, `c_info_row`, `c_card` (`c_panel` is the same with its arguments swapped), `c_chip`, `c_prog`, `c_menu`, `c_btn`, `c_ib` (the icon layout of `btn`) and `c_hold` (a `danger` button with a sweeping fill). Older names (`tog`, `nm`, `ro`, `pw`, `kv`, `bar1`, `seg5`, `field`) are only aliases of these. A new screen adds a variant here, never a private copy.
...
**UI size.** The shared slider (see Component Library) from **50% to 400%** with the value in `readout-md` (default 100%). It has four steps, 50%, 100%, 200% and 400%, evenly spaced, and the screen places the thumb on a logarithmic scale between them, so the everyday range from 50 to 200% is not squeezed into the left corner. A `touch`-sized **minus** and **plus** button on either side step it by 5% for precision. UI size scales the complete token set, including text, spacing, controls and their touch areas; `touch` is 48px at 100%, not a hard-coded minimum outside the scale. The arena is not scaled, it just gets what is left, so it shrinks as the UI grows. The screen is the preview: the change is seen on the Settings screen as it happens. Above about 200% the side panels no longer fit beside the arena, so they open over it one at a time; that layout is not drawn yet.
