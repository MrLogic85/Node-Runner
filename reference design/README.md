...
**Contents:** `index.html` (open this first: every component with a screenshot and a live preview), `icons/` (all icons as white SVG, see `icons.md`), `README.md` (this file, everything in one place), `tokens.json`, `bundle.css`, `library.md` (the `c_*` controls), `components/<Name>/` (README.md and a self-contained preview.html per component) and `screenshots/<Name>.png`.
...
Type: `heading` and `stage` in Chakra Petch, `body`, `label` and `caption` in Barlow, every changing number in `readout`, `readout-lg` or `readout-sm` in JetBrains Mono. These families are a proposal; bundle the files in the Godot project and swap the names in `tokens.json` to change them. Instructions and explanations are `body` (13px); helper lines under a control are `note` (11px) and `caption` (10px) is for one to three words in dense cards. Text is always one of the seventeen styles in tokens.json (see Component Library), and icons are `icon-sm`, `icon`, `icon-lg` or `icon-xl`.
...
- [ComponentLibrary](#componentlibrary): Every control, frame, text style and icon the game uses, in one place
...
*Foundations · Every control, frame, text style and icon the game uses, in one place*
