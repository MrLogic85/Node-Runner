...
**Contents:** `index.html` (open this first: every component with screenshot, preview and docs), `README.md` (this file, everything in one place), `tokens.json`, `bundle.css`, `library.md` (the `c_*` controls), `icons/` (white SVG icons) with `icons.md` and `_icons.html`, `components/<Name>/` (README.md and a standalone preview.html per component) and `screenshots/<Name>.png`.
