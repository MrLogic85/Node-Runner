...
Tapping a neuron rings it in `halo` and fills the bottom card with one sentence generated from its strongest links ("The left foot is down, so it pushes the hip forward."). The legend for thick and dashed edges appears once as a first-open hint, not permanently. The creature stays on the left, small, so the network stays tied to the body.